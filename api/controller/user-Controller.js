...
var UserController = {
 // User Registration
  ...
  UpdateUser(user,callback) {
        let insertQuery = 'Update `tbl_userdetails` set Name=?,Gender=?,Email=?,Aadhar=?,VoterId=?,PAN=? where Userid=?'
        return dbconfig.query(insertQuery, [user.name, user.gender, user.email, user.aadhar, user.voterid, user.pan, user.userid], (err, results) => {
          if (results.affectedRows > 0) {
        return callback(null, results)
          }
          else {
